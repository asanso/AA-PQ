...
export function verifiedGenesis(bytes, expectedSha256) {
  if (!/^[0-9a-f]{64}$/.test(expectedSha256 || '')) throw Error('Provide the original configuration SHA-256 from the trusted migration record');
  if (createHash('sha256').update(bytes).digest('hex') !== expectedSha256) throw Error('Original configuration fingerprint differs from the migration record');
  return JSON.parse(bytes.toString('utf8'));
}

async function main() {
  const [input,output,url,expectedGenesis,migrationBlock,activation,expectedConfigSha256,...extra]=process.argv.slice(2);
  if(!activation||!expectedConfigSha256||extra.length) throw Error('Usage: node scripts/schedule-nethermind-frames.mjs ORIGINAL_GENESIS NEW_GENESIS PRIVATE_RPC GENESIS_HASH MIGRATION_BLOCK UNIX_TIMESTAMP ORIGINAL_CONFIG_SHA256');
  ...
  const genesis=verifiedGenesis(await readFile(input),expectedConfigSha256);
