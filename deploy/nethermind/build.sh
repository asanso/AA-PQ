...
AA_IMAGE=${AA_IMAGE:-aa-pq-nethermind:frames-c9ad4b5-aa1-verify500k}
...
git -C "$AA_BUILD_DIR" apply --check "$AA_PATCH_DIR/verify-500k.patch"
git -C "$AA_BUILD_DIR" apply "$AA_PATCH_DIR/verify-500k.patch"
docker build --build-arg COMMIT_HASH="$AA_NETHERMIND_REF" \
  --build-arg VERSION=frames-c9ad4b5-aa1-verify500k -t "$AA_IMAGE" "$AA_BUILD_DIR"
