...
`eip8141-frame-txs-devnet7`, with `tohex-prefix.patch` applied. Source is retained
...
The migration target is the existing EF network, not a newly generated genesis.
...
rescheduling, stale heads, and timestamps less than five minutes away. It adds
only `eip8141PrototypeTime`; installing that file still requires an explicit
execution-node restart. `run-node.sh` is the EF-specific launcher retaining the
